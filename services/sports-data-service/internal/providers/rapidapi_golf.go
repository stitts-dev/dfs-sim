...
import (
	...
	// "github.com/stitts-dev/dfs-sim/shared/pkg/database" // TODO: Pass DB from service
...
func NewRapidAPIGolfClient(apiKey string, db *gorm.DB, cache types.CacheProvider, logger *logrus.Logger) *RapidAPIGolfClient {
...
func (c *RapidAPIGolfClient) GetPlayers(sport types.Sport, date string) ([]types.PlayerData, error) {
...
func (c *RapidAPIGolfClient) GetPlayer(sport types.Sport, externalID string) (*types.PlayerData, error) {
...
func (c *RapidAPIGolfClient) GetTeamRoster(sport types.Sport, teamID string) ([]types.PlayerData, error) {
...
func (c *RapidAPIGolfClient) GetCurrentTournament() (*GolfTournamentData, error) {
	...
	if err := c.saveTournamentToDatabase(context.Background(), tournament); err != nil {
...
func (c *RapidAPIGolfClient) saveTournamentToDatabase(ctx context.Context, tournamentData *GolfTournamentData) error {
	...
	if err == nil {
		...
		existingTournament.CurrentRound = tournamentData.CurrentRound
		existingTournament.CourseName = tournamentData.CourseName
		existingTournament.CoursePar = tournamentData.CoursePar
		existingTournament.CourseYards = tournamentData.CourseYards
		existingTournament.Purse = tournamentData.Purse
		existingTournament.CutLine = tournamentData.CutLine
		...
		}).Info("Updated existing golf tournament in database")

		return nil
	}

	if err != nil && err.Error() != "record not found" {
		return fmt.Errorf("error checking existing tournament: %w", err)
	}
	...
	c.logger.WithFields(logrus.Fields{
		...
		"tournament_name": tournamentData.Name,
		"sport_id":        sport.ID,
	}).Info("Created new golf tournament in database")

	// Now create a contest record for this tournament
	if err := c.createContestFromTournament(ctx, &tournament, sport); err != nil {
		c.logger.WithError(err).Warn("Failed to create contest from tournament")
	}

	return nil
}

// createContestFromTournament creates DFS contest records from golf tournament
func (c *RapidAPIGolfClient) createContestFromTournament(ctx context.Context, tournament *models.GolfTournament, sport *models.Sport) error {
	// Create contests for popular platforms (DraftKings, FanDuel)
	platforms := []struct {
		name      string
		salaryCap int
	}{
	...
	for _, platform := range platforms {
		// Check if contest already exists for this tournament and platform
		var existingContest types.Contest
		err := c.db.WithContext(ctx).Where("tournament_id = ? AND platform = ?", tournament.ID, platform.name).First(&existingContest).Error
		
		if err == nil {
			// Contest already exists, skip
			continue
		}
		...
		contest := types.Contest{
			...
			ContestType:  "gpp", // Default to GPP (tournaments)
			Name:         fmt.Sprintf("%s - %s", tournament.Name, strings.Title(platform.name)),
			SalaryCap:    platform.salaryCap,
			StartTime:    tournament.StartDate,
			IsActive:     tournament.Status == models.TournamentScheduled || tournament.Status == models.TournamentInProgress,
			TournamentID: &tournament.ID,
			ExternalID:   fmt.Sprintf("%s_%s", tournament.ExternalID, platform.name),
			PositionRequirements: types.GetPositionRequirements("golf", platform.name),
		}

		if err := c.db.WithContext(ctx).Create(&contest).Error; err != nil {
		...
		c.logger.WithFields(logrus.Fields{
			...
			"platform":        platform.name,
		}).Info("Created contest from golf tournament")
	}
...
func (c *RapidAPIGolfClient) savePlayersToDatabase(ctx context.Context, playersData []types.PlayerData, tournament *GolfTournamentData) error {
	...
	if err != nil {
		...
		return nil // Don't error out if tournament isn't saved yet
	}

	// Get existing contests for this tournament to link players
	var contests []types.Contest
	err = c.db.WithContext(ctx).Where("tournament_id = ?", dbTournament.ID).Find(&contests).Error
	if err != nil {
		c.logger.WithError(err).Warn("No contests found for tournament")
	}
	...
	for _, playerData := range playersData {
		...
		if err == nil {
			...
			if err := c.db.WithContext(ctx).Save(&existingPlayer).Error; err != nil {
				c.logger.WithError(err).WithField("player", playerData.Name).Warn("Failed to update player")
				continue
			}
		} else {
			// Create new player
			player := models.Player{
				...
				ProjectedPoints: 0, // Golf projections would come from separate service
				FloorPoints:     0,
				CeilingPoints:   0,
			...
			if err := c.db.WithContext(ctx).Create(&player).Error; err != nil {
				c.logger.WithError(err).WithField("player", playerData.Name).Warn("Failed to create player")
				continue
			}
			...
			c.logger.WithFields(logrus.Fields{
				...
				"sport_id":    sport.ID,
			}).Debug("Created new golf player in database")
		}

		// Create player entries for this tournament if they don't exist
		var playerEntry models.GolfPlayerEntry
		err = c.db.WithContext(ctx).Where("tournament_id = ? AND external_id = ?", dbTournament.ID, playerData.ExternalID).First(&playerEntry).Error
		...
		if err != nil {
			...
			if err := c.db.WithContext(ctx).Create(&entry).Error; err != nil {
				c.logger.WithError(err).WithField("player", playerData.Name).Warn("Failed to create player entry")
			}
		}
	}
	...
	c.logger.WithFields(logrus.Fields{
		...
		"sport_id":        sport.ID,
	}).Info("Saved golf players to database")
